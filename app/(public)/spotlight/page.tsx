  ...
  return (
    <div className="sgc-page">
      ...
      <style>{`
        ...
        .si-tile-image { height: 160px; background: rgba(217,119,87,0.07); display: flex; align-items: center; justify-content: center; position: relative; overflow: hidden; flex-shrink: 0; }
          ...
          <>
            <div className="si-grid">
              {articles.map((article: any) => {
                ...
                const imgSrc = filename
                  ...
                  : editorialPlaceholder("spotlight");
                ...
                return clickable ? (
                  <a key={article.ed_cal_id} href={`/spotlight/${article.slug}`} className="si-tile">{tileContent}</a>
                ) : (
            ...
            {totalPages > 1 && (
              <div className="si-pagination">
                <a href={`/spotlight?page=${currentPage - 1}`} className={`si-page-btn${currentPage === 1 ? " disabled" : ""}`}>← Prev</a>
                {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                  <a key={p} href={`/spotlight?page=${p}`} className={`si-page-btn${p === currentPage ? " active" : ""}`}>{p}</a>
                ))}
                <a href={`/spotlight?page=${currentPage + 1}`} className={`si-page-btn${currentPage === totalPages ? " disabled" : ""}`}>Next →</a>
              </div>
            )}
          </>
        )}

...
type Props = {
  pantheletes: PlayerData[];
  honorRoll: PlayerData[];
...
function RadarChart({ data, domDim, size = 240 }: { data: number[]; domDim: string; size?: number }) {
  ...
  useEffect(() => {
    ...
    ctx.scale(dpr, dpr);
    const cx = size / 2, cy = size / 2, r = (size / 2) * 0.68, n = 6;
    [0.25, 0.5, 0.75, 1].forEach((scale) => {
      ctx.beginPath();
      for (let i = 0; i < n; i++) {
        const angle = (Math.PI * 2 * i) / n - Math.PI / 2;
        const x = cx + r * scale * Math.cos(angle), y = cy + r * scale * Math.sin(angle);
        i === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y);
      }
      ctx.closePath(); ctx.strokeStyle = "rgba(255,255,255,0.07)"; ctx.lineWidth = 0.5; ctx.stroke();
    });
    for (let i = 0; i < n; i++) {
      const angle = (Math.PI * 2 * i) / n - Math.PI / 2;
    ...
    data.forEach((v, i) => {
      ...
      const x = cx + r * pct * Math.cos(angle), y = cy + r * pct * Math.sin(angle);
      i === 0 ? ctx.moveTo(x, y) : ctx.lineTo(x, y);
    });
    ctx.closePath(); ctx.fillStyle = domColor + "22"; ctx.fill();
    ctx.strokeStyle = domColor; ctx.lineWidth = 2; ctx.stroke();
    data.forEach((v, i) => {
      const angle = (Math.PI * 2 * i) / n - Math.PI / 2;
      const pct = Math.min(v, 100) / 100;
      const x = cx + r * pct * Math.cos(angle), y = cy + r * pct * Math.sin(angle);
      ctx.beginPath(); ctx.arc(x, y, 4, 0, Math.PI * 2);
      ctx.fillStyle = DC[DIMS[i]] ?? "#888"; ctx.fill();
    });
    DIMS.forEach((d, i) => {
      const angle = (Math.PI * 2 * i) / n - Math.PI / 2;
      const lx = cx + (r + 18) * Math.cos(angle), ly = cy + (r + 18) * Math.sin(angle);
      ctx.font = `700 9px 'Plus Jakarta Sans', sans-serif`;
      ctx.fillStyle = DC[d]; ctx.textAlign = "center"; ctx.textBaseline = "middle";
...
// ── Roll section — handles Panthelete + Honor Roll dividers ──
function RollTable({
  pantheletes,
  honorRoll,
  allPlayers,
  ...
  // Group honor roll by sport
  const sportGroups: Record<string, PlayerData[]> = {};
  for (const p of honorRoll) {
    if (!sportGroups[p.sport]) sportGroups[p.sport] = [];
    sportGroups[p.sport].push(p);
  }
  const sportsSorted = Object.keys(sportGroups).sort();
  ...
  function renderRow(p: PlayerData, rank: number, globalIdx: number) {
    const pcts = pctOfTotal(p.c);
    return (
      <tr key={globalIdx}
        style={{
          borderBottom: "1px solid #e0d8d0",
          background: p.qualified ? "rgba(232,180,76,0.05)" : "transparent",
        }}
      >
        <td style={{ padding: "10px 8px", fontSize: 10, fontWeight: 800, color: "#8a8580" }}>{rank}</td>
        <td style={{ padding: "10px 8px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <button
              onClick={() => { onSpotlight(globalIdx); window.scrollTo({ top: 0, behavior: "smooth" }); }}
              style={{ fontFamily: "'DM Serif Display', serif", fontSize: 15, color: "#3d3935", display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", cursor: "pointer", padding: 0, textAlign: "left" }}
            >
        ...
        <td style={{ padding: "10px 8px" }}>
          <div style={{ display: "flex", height: 14, borderRadius: 3, overflow: "hidden" }}>
            {DIMS.map((d, j) => (
              <div key={d} style={{ width: `${pcts[j]}%`, height: "100%", background: DC[d] }} title={`${d}: ${pcts[j]}%`} />
            ))}
          </div>
        </td>
        <td style={{ padding: "10px 8px" }}>
    ...
    <table style={{ width: "100%", borderCollapse: "collapse" }}>
      <thead>
        <tr style={{ borderBottom: "1px solid #e0d8d0" }}>
          {["#", "Name", "Sport", "Legacy shape", "Dominant", "Score"].map((h, i) => (
            <th key={h} style={colStyle(i)}>{h}</th>
          ))}
        </tr>
      </thead>
      <tbody>

        {/* ── Panthelete section header ── */}
        {pantheletes.length > 0 && (
          <tr>
            <td colSpan={6} style={{
              padding: "14px 8px 8px",
              background: "rgba(232,180,76,0.06)",
              borderBottom: "2px solid rgba(232,180,76,0.4)",
              borderTop: "2px solid rgba(232,180,76,0.4)",
            }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.14em", textTransform: "uppercase", color: "#8B6914" }}>
                  ★ SGC Pantheon — Pantheletes
                </span>
                <span style={{ fontSize: 9, color: "#8B6914", opacity: 0.6 }}>
                  Score ≥ {THRESHOLD.toLocaleString()} · Breadth ≥ 4 dims · No zeros
                </span>
              </div>
            </td>
          </tr>
        ...
        {pantheletes.map((p) => {
          globalRank++;
          const idx = allPlayers.findIndex(pl => pl.slug === p.slug);
          return renderRow(p, globalRank, idx >= 0 ? idx : globalRank - 1);
        })}

        {/* ── Honor Roll section header ── */}
        {honorRoll.length > 0 && (
          <tr>
            <td colSpan={6} style={{
              padding: "14px 8px 8px",
              background: "rgba(61,57,53,0.04)",
              borderBottom: "2px solid rgba(61,57,53,0.18)",
              borderTop: "3px solid rgba(61,57,53,0.25)",
            }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.14em", textTransform: "uppercase", color: "#5a5550" }}>
                  SGC Honor Roll
                </span>
                <span style={{ fontSize: 9, color: "#8a8580" }}>
                  Top performers by sport · sorted alphabetically by sport, score within
                </span>
              </div>
            </td>
          </tr>
        ...
        {/* ── Honor Roll rows grouped by sport ── */}
          ...
          <React.Fragment key={sport}>
            ...
            <tr>
              <td colSpan={6} style={{
                padding: "8px 8px 6px",
                borderBottom: "1px solid #e0d8d0",
                borderTop: "1px solid rgba(61,57,53,0.08)",
                background: "rgba(61,57,53,0.02)",
              }}>
                <span style={{ fontSize: 8, fontWeight: 800, letterSpacing: "0.12em", textTransform: "uppercase", color: "#8a8580" }}>
                  {sport}
                </span>
              </td>
            </tr>
            {/* Sport players */}
            {sportGroups[sport].map((p) => {
              globalRank++;
              const idx = allPlayers.findIndex(pl => pl.slug === p.slug);
              return renderRow(p, globalRank, idx >= 0 ? idx : globalRank - 1);
            })}
...
export default function PantheonClient({
  pantheletes, honorRoll, isIllustrative, memberTier, canLegacy, initialSlug,
}: Props) {
  // Combine for spotlight nav and explorer
  const allPlayers = [...pantheletes, ...honorRoll];

  const initialIdx = initialSlug
    ? Math.max(0, allPlayers.findIndex((p) => p.slug === initialSlug))
    : 0;
  ...
  useEffect(() => {
    setCompareB(Math.min(2, allPlayers.length - 1));
  }, [allPlayers.length]);
  ...
  const pantheleteCount = pantheletes.length;
  const spotPlayer = allPlayers[spotIdx] ?? allPlayers[0];
  const sports  = [...new Set(allPlayers.map((p) => p.sport))].sort();
  const statuses = [...new Set(allPlayers.map((p) => p.status))].sort();
  ...
  return (
    <div style={s.page}>
      ...
      {/* ── BACK LINK ── */}
      {initialSlug && (
        <div style={{ padding: "12px 48px 0", background: "#f5ede4" }}>
          <a
            href={`/player/${initialSlug}`}
            style={{ fontSize: "0.8rem", color: "var(--slate-ghost)", textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 6, transition: "color 0.15s" }}
      ...
      <div style={s.section}>
        ...
        {canLegacy ? (
            ...
            <div style={{ padding: "12px 32px 16px", display: "flex", gap: 6, alignItems: "center" }}>
              {allPlayers.map((_, i) => (
                <button key={i}
                  ...
                  aria-label={`View ${allPlayers[i].name}`}
                />
              ))}
          ...
          <div style={{ background: "rgba(139,34,82,0.04)", border: "1px solid rgba(139,34,82,0.15)", borderRadius: 10, padding: "20px 24px", display: "flex", alignItems: "center", gap: 12, fontSize: 13, color: "#5a5550" }}>
            <span>🔒</span>
            <span>
              Full Panthelete dimension profiles — radar charts, recipe breakdowns, and compare tools —
              are available with{" "}
              <a href="/membership" style={{ color: "#8B2252", fontWeight: 700, textDecoration: "none" }}>The Legacy →</a>
            </span>
          </div>
        )}
      </div>
      ...
      <div style={s.section}>
        ...
        <RollTable
          pantheletes={pantheletes}
          honorRoll={honorRoll}
          allPlayers={allPlayers}
          onSpotlight={setSpotIdx}
        />
        <div style={{ fontSize: 11, color: "#8a8580", fontStyle: "italic", marginTop: 14 }}>
          Pantheletes ranked by score · Honor Roll by sport then score · Percentages only
        </div>
      </div>
      ...
      <div style={s.section}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 14 }}>
          {DIMS.map((dim) => {
            ...
            const top5 = [...allPlayers].sort((a, b) => b.c[idx] - a.c[idx]).slice(0, 5);
            return (
              <div key={dim} style={{ background: "white", border: "1px solid #e0d8d0", borderRadius: 10, overflow: "hidden" }}>
                <div style={{ padding: "11px 16px", borderBottom: "1px solid #e0d8d0", background: DC[dim] + "08" }}>
                  <span style={{ fontSize: 8, fontWeight: 800, letterSpacing: "0.06em", textTransform: "uppercase", padding: "3px 9px", borderRadius: 8, color: "white", background: DC[dim] }}>{dim}</span>
                </div>
                {top5.map((p, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 16px", borderBottom: i < 4 ? "1px solid rgba(224,216,208,0.5)" : "none" }}>
                    ...
                    <span style={{ fontSize: 9, fontWeight: 800, padding: "2px 8px", borderRadius: 8, color: "white", background: DC[dim] }}>{p.c[idx]}%</span>
                  </div>
                ))}
              </div>
            );
      ...
      {canLegacy && (
        <div style={s.section}>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12, gap: 0 }}>
            {(["radar", "recipe"] as const).map((v) => (
              <button key={v}
                ...
                style={{ fontSize: 10, fontWeight: 700, padding: "5px 14px", background: compareView === v ? "rgba(61,57,53,0.1)" : "transparent", border: "1px solid #e0d8d0", borderRadius: v === "radar" ? "8px 0 0 8px" : "0 8px 8px 0", color: compareView === v ? "#3d3935" : "#8a8580", cursor: "pointer", fontFamily: "'Plus Jakarta Sans', sans-serif", marginLeft: v === "radar" ? 0 : -1 }}
              >
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
            {([{ idx: compareA, setIdx: setCompareA }, { idx: compareB, setIdx: setCompareB }] as const).map(({ idx, setIdx }, side) => {
              const p = allPlayers[idx] ?? allPlayers[0];
              return (
                <div key={side} style={{ background: "#1a1714", borderRadius: 12, overflow: "hidden", border: "1px solid rgba(255,255,255,0.06)" }}>
                  <div style={{ padding: "14px 20px", borderBottom: "1px solid rgba(255,255,255,0.07)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    ...
                    <span style={{ fontSize: 9, fontWeight: 800, padding: "3px 10px", borderRadius: 8, color: "white", background: DC[p.dom as Dim] ?? "#888" }}>{p.dom}</span>
                  </div>
                  {compareView === "radar" ? (
                    <div style={{ padding: "16px 20px" }}><RadarChart data={p.c} domDim={p.dom} size={220} /></div>
                  ) : (
                    <div style={{ padding: "16px 20px" }}>
                      <div style={{ display: "flex", height: 16, borderRadius: 4, overflow: "hidden", marginBottom: 8 }}>
                        {DIMS.map((d, i) => { const pcts = pctOfTotal(p.c); return <div key={d} style={{ width: `${pcts[i]}%`, height: "100%", background: DC[d] }} title={`${d}: ${pcts[i]}%`} />; })}
                      </div>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
                        {DIMS.map((d, i) => { const pcts = pctOfTotal(p.c); return (
                  ...
                  <div style={{ padding: "10px 20px", borderTop: "1px solid rgba(255,255,255,0.07)", display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <div style={{ marginLeft: "auto" }}>
                      {p.qualified
                        ...
                        : <span style={{ fontSize: 9, color: "rgba(255,255,255,0.35)", fontStyle: "italic" }}>{p.score.toLocaleString()} / 1,875 needed</span>
                      }
                    </div>
                  </div>
                </div>
      ...
      <div style={s.section}>
        ...
        <div style={s.expWrap}>
          ...
          <div style={{ fontSize: 8, fontWeight: 800, letterSpacing: "0.12em", textTransform: "uppercase", color: "#8a8580", marginBottom: 8 }}>Preset stories</div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 7, marginBottom: 18, alignItems: "center" }}>
            {Object.entries(PRESETS).map(([key, preset]) => (
              <button key={key} onClick={() => applyPreset(key)}
                style={{ ...s.preset, background: activePreset === key ? "#d97757" : "white", border: activePreset === key ? "1px solid #d97757" : "1px solid #ccc5ba", color: activePreset === key ? "white" : "#5a5550" }}
              >
          ...
          <div style={{ fontSize: 8, fontWeight: 800, letterSpacing: "0.12em", textTransform: "uppercase", color: "#8a8580", marginBottom: 10 }}>Minimum dimension %</div>
            ...
            {DIMS.map((d, i) => (
              <div key={d} style={{ background: "white", border: "1px solid #e0d8d0", borderRadius: 10, padding: "12px 14px" }}>
                ...
                <input type="range" min={0} max={100} step={5} value={sliderVals[i]}
                  onChange={(e) => { const next = [...sliderVals]; next[i] = Number(e.target.value); setSliderVals(next); setActivePreset(null); }}
          ...
          <div style={{ display: "flex", gap: 16, marginBottom: 18, flexWrap: "wrap" }}>
            <div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                {sports.map((sp) => (
                  <button key={sp} onClick={() => toggleFilter(sportFilter, sp, setSportFilter)}
            ...
            <div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                {statuses.map((st) => (
                  <button key={st} onClick={() => toggleFilter(statusFilter, st, setStatusFilter)}
          ...
          <button onClick={() => runQuery()} style={s.runBtn}>Find Players →</button>
          {explorerResults !== null && (
            <div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: "#3d3935" }}>
                  {explorerResults.length} player{explorerResults.length !== 1 ? "s" : ""} match
                </span>
                <span style={{ fontSize: 11, color: "#8a8580", fontStyle: "italic" }}>{explorerLabel}</span>
              </div>
              {explorerResults.length === 0 ? (
                <div style={{ padding: 32, textAlign: "center", color: "#8a8580", fontStyle: "italic", fontSize: 13 }}>No players match these criteria — try loosening the filters.</div>
              ) : (
                explorerResults.map((p, i) => {
                  ...
                  return (
                    <a key={i} href={`/player/${p.slug}`} style={s.resultCard}>
                      ...
                      <span style={{ fontSize: 12, fontWeight: 700, color: "#3d3935", minWidth: 42, textAlign: "right", fontFamily: "monospace" }}>{p.score.toLocaleString()}</span>
                    </a>
                  );
                })
              )}
